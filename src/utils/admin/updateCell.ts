...
export default async function updateCells(sheet: GoogleSpreadsheetWorksheet, row: GoogleSpreadsheetRow, coloumnIndex: number, data: WorkerSalary, workerData: WorkerData) {
    const rowIndex = row.rowNumber - 1

    await sheet.loadCells({
      startRowIndex: rowIndex, endRowIndex: rowIndex + 5, startColumnIndex: coloumnIndex, endColumnIndex: coloumnIndex + 2
    })
    ...
    if (workerData.isOverWork) {
      ...
      overWorkSalaryCell.formula = `=${ranksSalary[workerData.rank].overWork || 0} * ${workerData.overWorkTime}`
    }

    if (workerData.rank === 'актёр' && workerData.gamesCount && workerData.gamesCount > 2) {
        overWorkSalaryCell.formula = `=${ranksSalary[workerData.rank].overWork || 0} * ${workerData.gamesCount - 2}`
    }
