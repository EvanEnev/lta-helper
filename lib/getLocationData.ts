...
export default async function getLocationData(dateString: string, locationName: string, sheet: GoogleSpreadsheetWorksheet, rows: GoogleSpreadsheetRow[], ) {
    ...
    const localeDate = date.toLocaleString('ru-RU', {
      ...
      for (let i = 6; i <= 9; i++) {
        ...
        workers.forEach((worker: string, index: number) => {
          ...
          const time = `${hours.includes(':') ? hours : hours + ':00'}−${
          ...
          workersAndRanks.push({ rank, name })
        })
}