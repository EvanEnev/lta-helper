...
export default async function PayrollsCreate({
  ...
  const query = `select
    ...
    w.rank,
    w.is_former,
    sum(value) + sum(coalesce(overwork, 0)) + sum(coalesce(one_games, 0) * 250) + sum(coalesce(two_games, 0) * 500) + sum(coalesce(three_games, 0) * 750) + sum(coalesce(actor_games, 0) * 250) as value,
    case
